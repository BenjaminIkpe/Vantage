...
0. **Briefing branch (`feat/proactive-briefing-langgraph`)** — core + tracing built and Codespace-validated; finish the admin **UI approve-button** (S6), then open the PR (let CI go green). Optional to merge before the panel, but a strong exhibit: the LangGraph **hybrid** ("loop for reactive, graph for proactive"), the **HITL** approval gate, and the **Phoenix** span tree of the graph run. It's the live answer to *"why didn't you use LangGraph?"* and *"where would this go next?"*.
