...
- Tools (**9 exposed via MCP**): reads `get_customer_profile`, `get_open_issues`, `summarise_issue_history` (any role); writes `update_issue` (support+admin), `create_next_action` / `update_next_action` (admin); **browse** `list_customers`, `list_issues`, `list_next_actions` (filterable + paginated, PR #21). Discovered via MCP; parameterised SQL; writes attributed via `users.keycloak_id = token sub`. Live in `mcp_server/`.
