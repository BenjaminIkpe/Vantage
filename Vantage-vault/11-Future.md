...
4. **Propose → approve → route.** AI drafts a `next_action` (or a Sev-N incident) in a *pending* state; an admin approves/rejects; the system writes the real `next_action` row via the existing tool and routes the work (Slack to a team channel, ticket-system webhook). Schema: a `next_action_drafts` table or a `pending` enum value on `next_actions`. Approval flow + audit trail.
