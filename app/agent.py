...
def _system_prompt(principal: Principal) -> str:
    return (
        ...
        "output looks unprofessional. **Before each tool call, briefly narrate why you're "
        "calling it in 1–2 plain sentences** (e.g. \"I'll look up Velocity Marketplace to get "
        "the customer id.\"). The UI captures that narration as a 'thinking' trace, separate "
        "from your final answer. Do NOT repeat the narration in the final answer itself — once "
        "all tool calls are done, write the clean answer for the user, not commentary about "
        "what you did. If a customer is not found, say so plainly; "
